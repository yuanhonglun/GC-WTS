    ...
    def Remove_Duplicates_RT(self, msp, RT_data, out_path, check_latin):
        ...
        for index_1, row in RT_data.iterrows():
            try:
                ...
                error_df.loc[len(error_df)] = [row[0], row[1], "WARNING: The RT value is out of the setting range.",
                                               np.nan]
            ...
            if row[0] not in name_df["Name"].values.tolist():
                ...
                if row[0] not in synon_df["Synon"].values.tolist():
                    ...
                    error_df.loc[len(error_df)] = [row[0], row[1],
                                                   "WARNING: The compound was not found in the provided MSP library.",
                                                   np.nan]
                    RT_data.drop(index=index_1, inplace=True)
                else:
                    temp_list = []
        ...
        for l in lines:
            ...
            if len(line2) > 1:
                if ';' in line2[1]:
                    ...
                    print('WARNING: The file format cannot be recognized')
