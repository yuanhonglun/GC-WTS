...
##################
def calulate_lwma(x, weights, no_smooth_number):
    '''
...
def lwma(local_df, n):
    ...
    for ion in local_df.columns.values:
        smooth = local_df_padding[ion].rolling(window=n, min_periods=n, center=True).apply(calulate_lwma,
                                                                                           args=(
                                                                                               weights,
...
def find_peak(ion_list, smooth_df, noise_df, filter_factor):
    peak_dic = {}
    con_peak_dic = {}
    NF = pre_de_redundancy_peak(smooth_df, ion_list)
    ...
    for ion in ion_list:
        ...
        peak_df = trailing_peak_filtration(peak_df, smooth_df, ion)
        ...
        smooth_df_final = baseline_correction(peak_df, smooth_df, ion, smooth_df_final)
        # smooth_df_final.to_excel('./results/smooth_df_final_after_ba.xlsx')
        peak_df = de_redundancy_peak(smooth_df_final, smooth_df, peak_df, NF, ion)
        ...
        for peak in peak_df.index.values:
            ...
            new_apex_rt, new_apex_i = second_order_fit_X(
...
def trailing_peak_filtration(peak_df, smooth_df, ion):
    """
    删除拖尾峰和识别左右错误的峰
    """
    for peak in peak_df.index.values:
...
def second_order_fit_X(x1, y1, x2, y2, x3, y3):
...
def decon(ion_list, smooth_df_final, con_peak_dic):
    ...
    for ion in ion_list:
        if con_peak_dic[ion].shape[0] == 0:
            ...
            for left_value in new_raw_left_list:
                ...
                popt, pcov = curve_fit(Gaussian, x, y, bounds=(bound_min, bound_max), p0=para_list,
                                       maxfev=100000)
...
def calculate_sv(smooth_df_final, peak_dic, decon_data_df, decon_peak_dic):
    keys_to_delete = []
    ...
    for ion in peak_dic.keys():
        ...
        #################
        peak_dic[ion] = sharpness(peak_dic[ion], data_df)
    ...
    for ion in decon_peak_dic.keys():
        ...
        for peak in decon_peak_dic[ion].index.values:
            ...
            #################
            peak_df = sharpness(peak_df, data_df)
...
def group_peak(peak_dic, decon_peak_dic, smooth_df, smooth_df_final, decon_data_df, wid, sigma, bin_num,
               group_method):
    ...
    if group_method == 0:
        matched_wave, peak_group_df = MSDIAL(matched_wave, wid, sigma, peak_dic, decon_peak_dic)
    elif group_method == 1:
        matched_wave, peak_group_df = AMDIS(matched_wave)
...
def peak_group_df_add_retention_infor(peak_group_df, RI_Presentation, standard_df, RI_min, RI_max):
    """
    peak_group_df: 峰分组信息
    ...
    for i, row in peak_group_df.iterrows():
        ...
        if RI_Presentation == "Kovats":
            peak_group_df.loc[i, "Kovats_RI"] = RT_to_Kovats_RI_transform(rt_sample, standard_df, RI_min,
                                                                               RI_max)
        elif RI_Presentation == "Fiehn":
            peak_group_df.loc[i, "Fiehn_RI"] = RT_to_Fiehn_RI_transform(rt_sample, standard_df)
    return peak_group_df

def MSDIAL(matched_wave, wid, sigma, peak_dic, decon_peak_dic):
    """
    MSDIAL找物质
    """
    sg_list = []
...
def remove_content_from_list(lst, content):
...
def weighted_dot_product_distance(compare_df):
...
def euclidean_intensity(x, y, k, b):
    '''y = kx + b '''
    return math.fabs(b + k * (x - 0) - y) / (math.sqrt(k * k + 1))

def line_between_dots(x1, y1, x2, y2):
    '''y = kx + b 返回 k,b'''
    if x1 == x2:
...
def line(x, k, b):  # 定义拟合函数形式
    return k * x + b

def base_line(rt_list, intensity_list):
    para, pcov = curve_fit(line, rt_list, intensity_list)
    y_fitted = line(rt_list, para[0], para[1])  # 画出拟合后的曲线
    # plt.plot(rt_list, y_fitted, '-b')
    return para[0], para[1], y_fitted

def baseline_correction(peak_df, smooth_df, ion, smooth_df_final):
    base_line_rt = []
    base_line_intensity = []
    list_now = []
    for peak_line in peak_df.itertuples():
        ...
        k, b = line_between_dots(min2dots.index[0], min2dots.iloc[0, 0], min2dots.index[1],
                                      min2dots.iloc[1, 0])
        euclidean_intensity_list = []
        for rt, intensity in son_df.iterrows():
            euclidean_intensity_list.append(intensity.values[0] - line(rt, k, b))
        ...
        ##计算全部点到基线的距离
        k1, b1, y_fitted = base_line(son_son_df.index, son_son_df.iloc[:, 0])
        for rt1, intensity1 in son_df.iterrows():
            new_intensity = intensity1.values[0] - line(rt1, k1, b1)
            if new_intensity >= 0:
...
############
def pre_de_redundancy_peak(smooth_df, ion_list):
    """
    去除冗余峰
...
def de_redundancy_peak(smooth_df_final, smooth_df, peak_df, NF, ion):
    index_list = []
...
############################
def RT_similarity(rt_s, rt_l, window, level_factor, max_penalty):
    """
    NIST中采用的比较RI相似性方式
    rt_s:样品rt.
    rt_l：库中rt.
...
def RI_similarity(ri_s, ri_l, ri_window, ri_window_scale, level_factor, max_penalty):
...
def New_Compound(group_rt_s, row, score_df, results_df, min_ions_number: int, limit_of_score: float):
...
def species_pool_opt(results_df, species_pool, species_pool_coefficient=0.3,
...
def mass_spectra_comparison_plot(group_rt_s, compound, compare_df_1):
    """
    FUNCTION：生成质谱对比图
    group_rt_s: 分组名称
    compound:库中化合物名称
    ...
    #     plt.tight_layout()
    #
    #     compound_name = validateTitle(str(compound))
    #
    #     #plt.savefig('./results/comparison_plot/{}.png'.format(str(group_rt_s) + "_" + compound_name))
    #     plt.close()

def peak_group_identification(peak_group_df,
                              smooth_df,
                              smooth_df_final,
                              meta,
                              RT_df,
    ...
    for group, row in peak_group_df.iterrows():
        ...
        if retention_score_mode == "None":
            ...
            left_idx = find_nearest(RT_df["RI"].values.tolist(), group_ri - search_wid)
            # print("left = ", left_idx)
            # print(group_rt_min + search_wid)
            right_idx = find_nearest(RT_df["RI"].values.tolist(), group_ri + search_wid)
            # print("right = ", right_idx)
            compound_list = RT_df.iloc[left_idx: right_idx, :].index.values.tolist()
        elif retention_score_mode == "RT":
            left_idx = find_nearest(RT_df["RT"].values.tolist(), group_rt_min - search_wid)
            # print("left = ", left_idx)
            # print(group_rt_min + search_wid)
            right_idx = find_nearest(RT_df["RT"].values.tolist(), group_rt_min + search_wid)
            # print("right = ", right_idx)
            compound_list = RT_df.iloc[left_idx: right_idx, :].index.values.tolist()
        # print("compound_list_start = ", compound_list)
        rt_index = find_nearest(smooth_df.index.values.tolist(), group_rt_s)
        ...
        if peak_group_search == 1:
            compare_df = row["ions"]
            # else:
            n = find_nearest(smooth_df.index.values.tolist(), group_rt_s)
        ...
        if len(compound_list) > 0:
            for compound in compound_list:
                ...
                MS_score = weighted_dot_product_distance(compare_df_1)
                R_MS_score = weighted_dot_product_distance(r_match_compare_df)
                R_direct_MS_score = weighted_dot_product_distance(R_direct_compare_df_1)
                ...
                if retention_score_mode == "None":
                    ...
                    if 'Kovats_RI' or 'Fiehn_RI' in peak_group_df.columns:
                        ...
                        if type(ri_l) not in [float, int, np.float64, np.int64] or ri_l == np.nan:
                            ...
                            Retention_score, delta_ri = RI_similarity(ri_s,
                                                                           ri_l,
                                                                           window,
                                                                           window_scale,
                                                                           inaccurate_ri_level_factor,
                    ...
                    if 'RT' in RT_df.columns:
                        ...
                        if type(rt_l) not in [float, int, np.float64, np.int64] or rt_l == np.nan:
                            ...
                            Retention_score, delta_rt = RT_similarity(rt_s, rt_l, window, level_factor,
                                                                           max_penalty)
        ...
        if len(score_df) > 0:
            results_df = New_Compound(group_rt_s, row, score_df, results_df,
...
def RT_to_Fiehn_RI_transform(rt_sample, standard_df, Fiehn_RI_list=None):
    ...
    if rt_sample <= standard_df['RT (min)'][1]:
        z1 = linear_fit(C8_RT, C9_RT, C8_RI, C9_RI)
    elif rt_sample >= standard_df['RT (min)'][11]:
        z1 = linear_fit(C28_RT, C30_RT, C28_RI, C30_RI)
    else:
...
def linear_fit(arg0, arg1, arg2, arg3):
    x = np.array([arg0, arg1])
    y = np.array([arg2, arg3])
    return np.polyfit(x, y, 1)

def fill_in_missing_data(old_df):
    """
...
def export_msp(group_rt_s, peak_group_df):
...
def Main(total_result_pkl, peak_group_pkl, filename, smooth_value, peak_filt_value, group_peak_factor, msp,
         chooseinfo, RTRIinfo,
    ...
    if total_result is None:
        ...
        if peak_group_pkl != '':
            ...
            if isinstance(filename, pd.DataFrame):
                ...
                if filename.endswith(".mzML"):  # liunote
                    df = read_data_mzML(filename)
                elif filename.endswith(".cdf"):
                    df = read_data(filename)
            RT_max = max(df.index) / 60
            # yuannote: lwma里的5是向导页面1里的平滑系数
            smooth_df = lwma(df, smooth_value)  # liunote
            # yuannote: derivative里的10是指当ff等参数接近0时，用10替换，该参数默认10即可，不做输入接口
            smooth_df, noise_df = derivative(smooth_df, 10)
            ion_list = df.columns
            # ion_list = [41, 42, 43, 44, 45, 46, 47, 48]
            # del df
            # yuannote: find_peak里的10是是向导页面1里的峰过滤系数
            peak_dic, con_peak_dic, smooth_df_final = find_peak(ion_list, smooth_df, noise_df,
                                                                     peak_filt_value)  # todo:1 -> 10 #liunote
            decon_peak_dic, decon_data_df = decon(ion_list, smooth_df_final, con_peak_dic)
            peak_dic, decon_peak_dic = calculate_sv(smooth_df_final, peak_dic, decon_data_df, decon_peak_dic)
            # yuannote: group_peak里倒数第二个数字：0.5（bin_num)，对应的是向导页面1里的峰聚类灵敏度
            #smooth_df.to_csv("C:/Users/86724/Desktop/smooth_df.csv")
            matched_wave, peak_group_df, quant_result_df = group_peak(peak_dic, decon_peak_dic, smooth_df,
                                                                           smooth_df_final,
                                                                           decon_data_df, 5,
        ...
        if qualitative_and_quantitative_analysis_result is None:
            ...
            if RT_df_filename != '':  # liunote
                ...
                qualitative_and_quantitative_analysis_result = peak_group_identification(peak_group_df,
                                                                                              smooth_df,
                ...
                peak_group_df = peak_group_df_add_retention_infor(peak_group_df, "Kovats", standard_df, RI_min,
                                                                       RI_max)
...
total_result_pkl = ''
peak_group_pkl = ''
filename = 'D:/work/GC方法/GC可视化/work/DataAnalysis/tomato0512.mzML'
smooth_value = 5
peak_filt_value = 10
group_peak_factor = 1
msp = 'D:/work/GC方法/GC可视化/work/DataAnalysis/Remove_Duplicates.msp'
chooseinfo = "RT"
RTRIinfo= 'D:/work/GC方法/GC可视化/work/DataAnalysis/test_RT.csv'
...
# Main(total_result_pkl, peak_group_pkl, filename, smooth_value, peak_filt_value, group_peak_factor, msp, chooseinfo, RTRIinfo,
...
if total_result is None:
    ...
    with open(r"C:\Users\86724\Desktop\total_result_test_811.pkl", "wb") as f:  # liunote
        pickle.dump(total_result, f)
