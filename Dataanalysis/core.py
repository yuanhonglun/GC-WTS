...
class DataAnalysis():
    ...
    def group_peak(self, peak_dic, decon_peak_dic, smooth_df, smooth_df_final, decon_data_df, wid, sigma, bin_num,
        ...
        for group, row in peak_group_df.iterrows():
            ...
            Quant_Ion = quant_df_index[0]
            if group in decon_data_df.index.values and decon_data_df.loc[
                group, lambda d: d.columns.str.contains(f'{Quant_Ion}_peak')].isnull().values.any() == False:
                Quant_Ion_try_list = []
                for Quant_Ion_try in quant_df_index:
                    if (group in decon_data_df.index.values and decon_data_df.loc[
                        group, lambda d: d.columns.str.contains(
                            f'{Quant_Ion_try}_peak')].isnull().values.any()) or (
                            group in decon_data_df.index.values == False):
        ...
        # peak_group_df.to_excel('./results/group/peak_group_df.xlsx')
    ...
    def peak_group_identification(self, peak_group_df,
        ...
        for index, row in results_df.iterrows():
            if row["Best_match"].shape[0] > 0:
                results_df_filtered.loc[index, "Name"] = row["Best_match"].index.values[0]
                results_df_filtered.loc[index, "Score"] = row["Best_match"].iloc[0, 0]
        ...
        # results_df.to_excel('./results/score/final_results.xlsx')
        ...
        if total_result is None:
            ...
            if os.path.exists(peak_group_filename):
                with open(peak_group_filename, "rb") as f:
                    ...
                    smooth_df, smooth_df_final, decon_peak_dic, decon_data_df, peak_group_df = peak_group_result
            ...
            if peak_group_result is None:
                ...
                peak_group_result = smooth_df, smooth_df_final, decon_peak_dic, decon_data_df, peak_group_df, quant_result_df
...
# filename = './raw/tomato0512.mzML'
# smooth_value = 5
# peak_filt_value = 10
# group_peak_factor = 1
# msp = './raw/Remove_Duplicates.msp'
# chooseinfo = "基于RI"
# RTRIinfo = './raw/RI-230629.csv'
#
...
#
# a.Main(total_result_pkl, peak_group_pkl, filename, smooth_value, peak_filt_value, group_peak_factor, msp, chooseinfo,
#        RTRIinfo,
#        page3none_match_weight, page3none_r_match_weight, page3none_group_weight, page3none_direct_weight,
#        page3none_group_minimum_number_of_ions, page3none_sim_threshold, page3RT_search_wid,
#        page3RT_match_weight, page3RT_r_match_weight, page3RT_group_weight, page3RT_direct_weight,
